...
static BYTE xscan2kex[256] = {
    ...
    0x00,0x00,0x00,0x52, 0x00,0x00,0x00,0x5D, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    ...
    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x1D,0x00,0x00,0x00, 0x00,0x00,0x38,0x5B, 0x5C,0x00,0x00,0x00,
...
static WORD xchar2kex[256] = {
    ...
    0x00,0x00,0x00,0x1B9, 0x00,0x00,0x00,0x14A, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x02,

    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x10D,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x00,0xB4,0xB0,0xB2, 0xB3,0xB1,0xB8,0xB7, 0xB5,0x37,0xB9,0xB6,
    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x2A,0x2B, 0x2C,0x2D,0x00,0x12F,
    0x30,0x31,0x32,0x33, 0x34,0x35,0x36,0x37, 0x38,0x39,0x00,0x00, 0x00,0x00,0x00,0x00,

    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x100,0x00,0x00,0x00, 0x00,0x00,0x100,0x138, 0x138,0x00,0x00,0x00,
    0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x00, 0x00,0x00,0x00,0x1B6
};

DWORD k_translate_xkey(XKeyEvent* e, DWORD* ext, DWORD* mod)
{
    ...
    if (key==XK_ISO_Level3_Shift) key = XK_Alt_R;
...
void k_process_event(k_context* ctx)
{
    ...
    {
        DWORD button_id,ext,mod; int hot;
        ...
        case KeyPress:
            button_id = k_translate_xkey(&ev.xkey, &ext, &mod);
            if(mod&KMOD_CAPSLOCK) k_set_keyboard_modifiers(KMOD_CAPSLOCK, (ev.xkey.state&LockMask)!=0);
            if(mod&KMOD_NUMLOCK) k_set_keyboard_modifiers(KMOD_NUMLOCK, (ev.xkey.state&Mod2Mask)!=0);
            if(mod&KMOD_SCROLL) k_set_keyboard_modifiers(KMOD_SCROLL, (k_get_keyboard_modifiers()&KMOD_SCROLL)!=0);
            if(mod&~KMOD_LOCKMASK) k_set_keyboard_modifiers(mod,0);
            hot = k_check_hotkey(mod&KMOD_CTRALTSH?0:(button_id>>16));
            if(ctx->kbd_mode==1)
            {
                button_id = (button_id>>8)&0xFF00;
                if(ext) k_event_keypress(ctx, 0xE000);
            }
            if(ctx->kbd_mode==1 || (mod==0&&hot==0)) k_event_keypress(ctx, button_id);
            break;

        case KeyRelease:
            button_id = k_translate_xkey(&ev.xkey, &ext, &mod);
            if(mod&~KMOD_LOCKMASK) k_set_keyboard_modifiers(mod,1);
            k_check_hotkey(mod&KMOD_CTRALTSH?0:((button_id>>16)|0x80));
            if(ctx->kbd_mode==1)
            {
                button_id = (button_id>>8)&0xFF00;
                if(ext) k_event_keypress(ctx, 0xE000);
        ...
        case FocusIn:
            ...
            k_focus_in(ctx);
            ctx->focused = 1;
            if(ctx->window_w<=1 && ctx->window_h<=1) break;
            k_draw_window_intern(ctx,1);
            k_event_redraw(ctx);
            break;
